...
import { SplitPayment } from '@/types/api';
...
interface PaymentModalProps {
  ...
  onPaymentComplete: (
    ...
    method: string,
    ecentricDetails?: any,
    payments?: SplitPayment[] // Add split payments support
  ) => Promise<any>;
  items?: Array<{
    id?: number;
    name: string;
  ...
  const handleEcentricPayment = async () => {
    ...
    try {
      ...
      if (isApproved && response.resultCode === "01") {
        ...
        // Extract data from receiptBundle if available (receiptBundle has the actual transaction details)
        const receiptBundle = response.receiptBundle || {};
        const authCode = receiptBundle.AUTH_CODE || response.authCode || `AUTH-${transactionUuid.substring(0, 6)}`;
        const rrn = receiptBundle.RRN || response.rrn || transactionUuid.substring(0, 12);
        const maskedPan = receiptBundle.PAN || response.maskedPan || "************1234";
        const aid = receiptBundle.AID || response.aid || "A0000000031010";
        const terminalId = receiptBundle.TERMINAL_ID || response.terminalId || response.pebbleSerialNumber || "74100199";
        const cardType = receiptBundle.APP_LABEL || receiptBundle.APPLICATION_LABEL || response.cardType || "CARD";

        console.log("Extracted payment details from receiptBundle:", {
          authCode,
          rrn,
          maskedPan,
          terminalId,
  ...
  return (
    <>
            ...
            >
              {step === "method" ? (
                <View style={tw`p-4`}>
                  ...
                  <TouchableOpacity
                    style={tw`bg-purple-600 rounded-xl p-4 mb-4 border-2 border-purple-400`}
                    onPress={() => {
                  ...
                  >
                    <View style={tw`flex-row items-center justify-center`}>
                      ...
                      />
                      <GeistText
                        style={[tw`text-white font-bold`, typography.bodyBold]}
                      >
                        Split Payment
                      </GeistText>
                    </View>
                    <GeistText
                      style={[tw`text-white/90 text-center mt-1`, typography.caption]}
                    >
      ...
      <SplitPaymentModal
        ...
        total={total}
        onPaymentComplete={async (payments: SplitPayment[]) => {
          try {
            // Calculate total from payments
            const paymentTotal = payments.reduce((sum, p) => sum + p.amount, 0);
            // Determine primary payment type (first non-cash payment, or cash)
            const primaryType = payments.find(p => p.type !== "CASH")?.type || payments[0]?.type || "CASH";
            
            // Call onPaymentComplete with payments array and wait for completion
            await onPaymentComplete(paymentTotal, primaryType || "CASH", undefined, payments);
            ...
            throw error; // Re-throw so SplitPaymentModal knows there was an error
          }
        }}
        items={items}
        customer={customer}
            ...
            <View style={tw`w-1/2 p-6 border-r border-gray-200 bg-white`}>
              <View style={tw`flex-1`}>
                ...
                <View style={tw`mt-4`}>
                  ...
                  {/* Split Payment Option */}
                  <TouchableOpacity
                    style={tw`bg-purple-600 rounded-xl p-4 mt-4 border-2 border-purple-400`}
                    onPress={() => {
                      setShowSplitPaymentModal(true);
                    }}
                  >
